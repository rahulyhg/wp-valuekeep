...
<div class="col-md-<?php echo $col_type; ?> col-sm-<?php echo $col_type; ?>">
  ...
  <h2 class="singlepg-font">
    ...
    <i class="fa fa-eye"></i>
     <span> <?php 
      ...
      <i class="fa fa-user"></i> <span>Valuekeep</span>
  ...
  <?php if (is_single() && has_term( '', 'manual_kb_tag' )) { ?>
      <div class="tagcloud singlepgtag kbtag clearfix margin-btm-20 singlepg"><span><i class="fa fa-tags"></i> <?php echo esc_html__( 'Tags:', 'manual' ); ?></span><?php $string = ' ';
            ob_start();
            the_terms( $post_id , 'manual_kb_tag');
            $string .= ob_get_clean();
            $pieces = explode(",", $string);
            $results = array_unique($pieces);
