  ...
  <xsl:template name="content">
    ...
    <table class="widefat">
      ...
      <tfoot>
        ...
        <tr>
          <td colspan="3" class="manage-column"><xsl:value-of select="//REPORT/TRANSLATE/NOBROWSERDETECT"/></td>
          ...
          <xsl:if test="position() &gt; //REPORT/INDICATORS/PER_PAGE * (//REPORT/INDICATORS/CURRENT_PAGE - 1)">
            <xsl:if test="position() &lt;= //REPORT/INDICATORS/PER_PAGE * //REPORT/INDICATORS/CURRENT_PAGE">
              <tr>
                ...
                <td>
                  <xsl:if test="BROWSER/@tor = '1'">
                    <strong style="text-transform: uppercase;"><xsl:value-of select="//REPORT/TRANSLATE/TOR_NETWORK"/></strong>:
                  </xsl:if>
