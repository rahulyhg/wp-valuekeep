<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="content">
    <table class="widefat">
      <thead>
        <tr><td colspan="2"><h2><xsl:value-of select="//REPORT/TRANSLATE/SETTINGS_TITLE"/></h2></td></tr>
      </thead>
      <tbody>
        <tr>
          <td>
        ...
        <tr>
          ...
          <td>
            ...
            <div id="mystat_addip" style="display:none;background-color:#eee;padding:15px;border-radius:15px;margin-bottom:15px;">
              ...
              <button class="btn btn-small" onclick="addIp();return false;" style="margin-top:5px;width:100%;text-transform:uppercase;"><xsl:value-of select="//REPORT/TRANSLATE/NOIP_ADD"/></button>
            </div>
          ...
          <td>
            ...
            <div id="mystat_listmenu">
              ...
              <xsl:for-each select="//REPORT/MENU">
                <h4 style="font-weight: bold;margin-top: 10px;"><span><xsl:value-of select="TITLE"/></span></h4>
    ...
    <script type="text/javascript"><![CDATA[
      ...
      function repaintIp(){
        ...
        for(var i=0;i<ip.length;i++){
          var addr = ip[i].split('/');
          jQuery('#mystat_iplist').append('<div style="margin:5px;"><code>'+addr[0]+'</code>/<code>'+addr[1]+'</code><button onclick="delIp('+i+');return false;" class="btn btn-small" style="height:19px;line-height: 18px;margin-left: 10px;padding: 0px 8px;">X</button>'+'</div>');
        }
      }
      function delIp(i){
    ...
    <div style="text-align:center;padding: 30px 0;">
      <button class="btn btn-large" style="width:40%;text-transform:uppercase;" onclick="saveSettings();return false;" id="mystat_save"><xsl:value-of select="//REPORT/TRANSLATE/SAVE"/> <span class="spinner"></span></button>
    </div>
  </xsl:template>
</xsl:stylesheet>