...
class mystat_geolocation{
  ...
  private function readWord($pos) {
    ...
    $var = unpack('V', $this->read($pos - 1, 4));
    return self::wrap32($var[1]);
  }
  ...
  private function readByte($pos) {
    ...
    $var = unpack('C', $this->read($pos - 1, 1));
    return self::wrap8($var[1]);
  }
      ...
      switch ($afield) {
        ...
        case self::COUNTRY_CODE:
          if (!$done[self::COUNTRY_CODE]) {
            $var = $this->readCountryNameAndCode($pointer);
            $results[self::COUNTRY_CODE] = $var[1];
            $done[self::COUNTRY_CODE]    = true;
          }
          break;
        case self::COUNTRY_NAME:
          if (!$done[self::COUNTRY_CODE]) {
            $var = $this->readCountryNameAndCode($pointer);
            $results[self::COUNTRY_CODE] = $var[0];
            $done[self::COUNTRY_CODE]    = true;
          }
          break;
        ...
        case self::LATITUDE:
          if (!$done[self::LATITUDE]) {
            $var = $this->readLatitudeAndLongitude($pointer);
            $results[self::LATITUDE] = $var[0];
            $done[self::LATITUDE]    = true;
          }
          break;
        case self::LONGITUDE:
        ...
        case self::IDD_CODE:
          if (!$done[self::IDD_CODE]) {
            $var = $this->readIddAndAreaCodes($pointer);
            $results[self::IDD_CODE] = $var[0];
            $done[self::IDD_CODE]    = true;
          }
          break;
        case self::AREA_CODE:
          if (!$done[self::AREA_CODE]) {
            $var = $this->readIddAndAreaCodes($pointer);
            $results[self::AREA_CODE] = $var[1];
            $done[self::AREA_CODE]    = true;
          }
          break;
        case self::WEATHER_STATION_CODE:
          if (!$done[self::WEATHER_STATION_CODE]) {
            $var = $this->readWeatherStationNameAndCode($pointer);
            $results[self::WEATHER_STATION_CODE] = $var[1];
            $done[self::WEATHER_STATION_CODE]    = true;
          }
          break;
        case self::WEATHER_STATION_NAME:
          if (!$done[self::WEATHER_STATION_NAME]) {
            $var = $this->readWeatherStationNameAndCode($pointer);
            $results[self::WEATHER_STATION_NAME] = $var[0];
            $done[self::WEATHER_STATION_NAME]    = true;
          }
          break;
        case self::MCC:
          if (!$done[self::MCC]) {
            $var = $this->readMccMncAndMobileCarrierName($pointer);
            $results[self::MCC] = $var[0];
            $done[self::MCC]    = true;
          }
          break;
        case self::MNC:
        ...
        case self::MOBILE_CARRIER_NAME:
          if (!$done[self::MOBILE_CARRIER_NAME]) {
            $var = $this->readMccMncAndMobileCarrierName($pointer);
            $results[self::MOBILE_CARRIER_NAME] = $var[2];
            $done[self::MOBILE_CARRIER_NAME]    = true;
          }
          break;
        case self::ELEVATION:
