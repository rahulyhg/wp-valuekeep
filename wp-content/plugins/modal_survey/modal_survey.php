...
define( 'GRID_ITEMS' , '' );
define( 'MODAL_SURVEY_VERSION' , '1.9.7.4' );
define( 'MSDIRS' , '/' );
...
http://pantherius.com/modal-survey' );
		...
		**/
		...
		public function survey_records_shortcodes( $atts ) {
			...
			$sql_u = "SELECT username, email, name FROM " . $wpdb->base_prefix . "modal_survey_participants WHERE autoid = '" . $ssuid . "'";
		...
		public function survey_answers_shortcodes( $atts ) {
						...
						//display individual records end
							...
							$a_sql = $wpdb->get_results( $sql );
								foreach($a_sql as $key2=>$as) {
									...
									$uv_ans = "";
									if ( $aoptions[ 0 ] == "open" ) {
										if ( isset( $user_votes[ $ars->question_id ] ) ) {
											$uv_ans = explode( "|", $user_votes[ $ars->question_id ][ 0 ] );
											$user_votes[ $ars->question_id ] = array( $uv_ans[ 0 ] );
				...
				if ( $args[ 'data' ] == 'score' || $args[ 'data' ] == 'average-score' || $args[ 'data' ] == 'rating' ) {
					...
					foreach( $q_sql as $key1 => $ars ) {
						...
						if ( $args[ 'uid' ] != "false" ) {
							...
							if ( ! empty( $a_sql_u ) ) {
								...
								foreach( $a_sql as $key2o=>$aso ) {
									if ( isset( $user_votes[ $aso->question_id ] ) && ( $aso->autoid == $user_votes[ $aso->question_id ] ) ) {
										$a_sql[ $key2o ]->count = 1;
									}
									else {
										$a_sql[ $key2o ]->count = 0;										
						...
						foreach( $a_sql as $key2=>$as ) {
							...
							if ( isset( $aoptions[ 4 ] ) && $aoptions[ 4 ] > 0 ) {
					...
					if ( $args[ 'qid' ] == "" && $args[ 'aid' ] == "" && ! empty( $sdatas[ 0 ] ) ) {
					...
					if ( $args[ 'qid' ] != "" && $args[ 'aid' ] == "" ) {
						if ( ! is_numeric( $args[ 'qid' ] ) ) {
							if ( $args[ 'uid' ] != "false" ) {
								$fullrecords = modal_survey::survey_answers_shortcodes( 
									array ( 'id' => $args[ 'id' ], 'data' => 'full-records', 'style' => 'plain', 'uid' => ( isset( $_COOKIE[ 'ms-uid' ] ) ? $_COOKIE[ 'ms-uid' ] : '') , 'pure' => 'true'  )
								);
								foreach( $fullrecords as $fr ) {
									$category = "";
									preg_match_all( "/\[([^\]]*)\]/", $fr[ 'title' ], $cat );
			...
			if ( $args[ 'style' ] == 'piechart' || $args[ 'style' ] == 'barchart' || $args[ 'style' ] == 'doughnutchart' || $args[ 'style' ] == 'linechart' || $args[ 'style' ] == 'polarchart' || $args[ 'style' ] == 'radarchart' ) {
				...
				foreach( $q_sql as $key1 => $ars ) {
					if ( $args[ 'data' ] == 'full' || ( ( $key1 + 1 ) == $args[ 'qid' ] ) ) {
						...
						if (($key1+1)==$args['qid']&&$args['data']!='full') break;
					}
					if ( $args[ 'data' ] == 'score' || $args[ 'data' ] == 'average-score' || $args[ 'data' ] == 'rating' ) {
						if ( $key1 == count( $ars ) - 1 ) {
							$result .= '<div class="modal-survey-chart' . $key1 . ' ms-chart">';
						...
						if ( $args[ 'uid' ] != "false" ) {
							...
							$sql_u = "SELECT mspd.qid, mspd.aid FROM " . $wpdb->base_prefix . "modal_survey_participants_details mspd LEFT JOIN " . $wpdb->base_prefix . "modal_survey_participants msp on mspd.uid = msp.autoid WHERE mspd.sid = '" . $args[ 'id' ] . "' AND mspd.qid = '" . $ars->question_id . "' AND msp.id = '" . $args[ 'uid' ] . "' ORDER BY autoid ASC";
							$a_sql_u = $wpdb->get_results( $sql_u );
							if ( ! empty( $a_sql_u ) ) {
								foreach( $a_sql_u as $key2u=>$asu ) {
									$uv_ans = explode( "|", $asu->aid );
			...
			if ( $args[ 'data' ] == "score" || $args[ 'data' ] == "average-score" ) {
				if ( $args[ 'sort' ] == "asc" ) {
					usort( $sdatas[ 0 ], array( &$this, 'sortByCount' ));				
				}
				if ( $args[ 'sort' ] == "desc" ) {
					usort( $sdatas[ 0 ], array( &$this, 'sortByCount' ));	
			...
			$msplugininit_answer_array[ $args['id'] . '-' . $unique_key ] = array( "style" => array( "style" => $args[ 'style' ], "max" => $args[ 'max' ], "bgcolor" => $args[ 'bgcolor' ], "percentage" => $args[ 'percentage' ], "after" => $args[ 'after' ], "legend" => $args[ 'legend' ] ), "datas" => $sdatas );
			if ( $args[ 'compare' ] == "true" ) {
		...
		public function survey_shortcodes( $atts ) {
			...
			$soctit_temp = ( $atts[ 'socialtitle' ] == "" ? ( get_the_title() ) : $atts[ 'socialtitle' ] );
			$socdes_temp = ( $atts[ 'socialdesc' ] == "" ? ( $this->get_short_desc() ) : $atts[ 'socialdesc' ] );
			$survey[ 'social' ] = array( $social, get_option( 'setting_social_sites' ), $socialstyle, $socialpos, ( $atts[ 'sociallink' ] == "" ? ( home_url(add_query_arg(array(),$wp->request)) . '/' ) : $atts[ 'sociallink' ] ), ( $atts[ 'socialimage' ] == "" ? ( $this->get_featured_image() ) : $atts[ 'socialimage' ] ), strip_tags( $soctit_temp ) , strip_tags( $socdes_temp ), ( $fbappid != "" ? $fbappid : "" ) );
			...
			if ( $soa[18] == 1 ) {
				...
				if ( $args[ 'style' ] == 'flat' && in_array( $survey[ 'auto_id' ], $survey_viewed ) ) {
					return( htmlspecialchars_decode( $args[ 'message' ] ) );
				}
				else
				{
		...
		**/
		...
		global $wpdb;
			...
			if ( ! empty( $ms_session ) ) {
				$current_qid = $wpdb->get_var( $wpdb->prepare( "SELECT `qid` FROM " . $wpdb->base_prefix . "modal_survey_participants_details WHERE `sid` = %d AND `samesession` = %d ORDER BY `qid` DESC", $sid, $ms_session ) );
				return $current_qid;
			}
			else {
				return '-1';
		...
		function initialize_plugin_content() {
		global $msplugininit_array, $msplugininit_answer_array, $script;
			if ( $this->mscontentinit == "false" ) {
				$this->mscontentinit = "true";
				if ( ! empty( $script ) ) {
					echo $script;
				}
				if ( ! empty( $msplugininit_array ) ) {
