	...
	**/
		...
		**/
		...
		reset($postcharts);
			...
			<option " . selected( $value, 'full', false ) . " value='full'>Default</option>
			<option " . selected( $value, 'score', false ) . " value='score'>Score</option>
			<option " . selected( $value, 'rating', false ) . " value='rating'>Rating</option></select>";
		...
		**/
		function enqueue_admin_custom_scripts_and_styles() {
			...
			wp_enqueue_script('modal_survey_datatable', plugins_url( '/templates/assets/js/datatables.min.js', __FILE__ ) , array('jquery'), false, true);
			...
			wp_register_script('modal_survey_admin', plugins_url( '/templates/assets/js/modal_survey_admin.js', __FILE__ ) , array( 'jquery','modal_survey_timepicker_script','jquery-effects-drop','jquery-ui-accordion'), MODAL_SURVEY_VERSION, true );
		...
		**/		
		...
		global $wpdb;
			if ( $endcontent == "true" && ! empty( $sopts[ 144 ] ) && isset( $request[ 'endcontent' ] ) ) {
				...
				foreach( $fullrecords as $key=>$fr ) {
					...
					<div style='font-weight: bold;'>" . $fr[ 'title' ] . "</div>";
						foreach( $fr[ 'datas' ] as $ans ) {
							...
							$message .= "<div style='background-color:" . $bgcolor . "'>";
				...
				";
				if ( isset( $puid->autoid ) ) {
					$wpsendmessage = wp_mail( $sopts[ 144 ], __( 'Survey Completed', MODAL_SURVEY_TEXT_DOMAIN ), nl2br( $message ), $headers, $attachments );
				}
				if ( ! $wpsendmessage ) {
		...
		public function send_autoresponse( $content, $sender, $senderemail, $subject, $recipient, $sid ) {
			global $wpdb;
				...
				$headers[] = 'Content-Type: text/html; charset=UTF-8';
				$wpsendautoresponse = wp_mail( $recipient, $subject, $newcontent, $headers, $attachments );
		...
		if ( isset( $_REQUEST[ 'sspcmd' ] ) && $_REQUEST[ 'sspcmd' ] == "form" ) {
			...
			if ( isset( $_COOKIE[ 'ms-uid' ] ) ) {
				...
				if ( isset( $_REQUEST[ 'email' ] ) && ! empty( $_REQUEST[ 'email' ] ) ) {
					$ms_user_form_data[ 'email' ] = $_REQUEST[ 'email' ];
				...
				foreach( $options[ 'aid' ] as $key=>$aid_item ) {
					...
					if ( get_option( 'setting_save_votes' ) == 'on' && isset( $msuser_cookie ) ) {
						...
						if ( $check_user > 0 ) {
							...
							if ( $insdet == false ) {
								//START fix for Version 1.8.2 that provides new column: 'postid'
		...
		public function ajax_survey() {
			...
			if ( $_REQUEST['sspcmd'] == "getapiinfo" ) {
				...
				if ( $_REQUEST[ 'sspcmd' ] == "zip" ) {
					...
					$destination = sprintf( "%s" . MSDIRS . "exports" . MSDIRS . "modalsurvey-export.zip", dirname( __FILE__ ) );
