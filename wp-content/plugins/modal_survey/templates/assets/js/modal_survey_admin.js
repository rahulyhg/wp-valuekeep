(function( jQuery ){
	...
	surveysystem( document ).on( "click", ".add_condition", function(event) {
		...
		var newline_str = "<input type='hidden' class='packed_cond' value='" + JSON.stringify( newline ) + "'>If ";
			...
			surveysystem( this ).parent().parent().find(".added_conditions").append( "<div class='one_condition_line'><a href='#' class='remove_condition'><img class='modal_survey_tooltip' title='Remove Condition' src='"+sspa_params.plugin_url+"/templates/assets/img/delete.png'></a>" + newline_str + "</div>" );
	});
	...
	surveysystem("#"+survey_id+" .modal_survey_preview1004").spectrum({
		...
		surveysystem(".font_family").on("change", function(){
			...
			{
				if (!surveysystem("link[href='http://fonts.googleapis.com/css?family="+surveysystem(this).val()+"']").length) {
					surveysystem('head').append('<link rel="stylesheet" href="http://fonts.googleapis.com/css?family='+surveysystem(this).val()+'" type="text/css" />');
				}
	  ...
	  activate: function( event, ui ) {
        ...
        stop: function( event, ui ) {
		  ...
		  surveysystem("#"+survey_id+" #new_questions h3").each(function( index ) {
			surveysystem(this).html('<span class="ui-accordion-header-icon ui-icon ui-icon-triangle-1-e"></span>'+(index+1)+'. ' + sspa_params.languages.question);
		  })
        }
      });
};
			...
			ziptimer = setTimeout( function(){
				...
				clearTimeout( ziptimer );
					if ( response.toLowerCase().indexOf( "success" ) >= 0 ) {
						crtposhtml = 100;
						crtpos = 100;
						surveysystem( ".export-progress .process_text" ).html( "<a href='" + sspa_params.plugin_url + "/exports/modalsurvey-export.zip" + "'>ZIP Complete - Click here to Download</a>" );
						surveysystem( ".export-progress .survey_global_percent" ).css( "width", crtposhtml + "%" );
						var percent_number_step = surveysystem.animateNumber.numberStepFactories.append( "%" );
						surveysystem( ".export-progress .perc" ).prop( 'number', surveysystem( ".export-progress .perc" ).text().replace( "%", "" ) ).animateNumber({
							number: crtpos,
