...
if ( isset( $_REQUEST[ 'msuid' ] ) ) {
	...
	$check_session = "SELECT aid FROM " . $this->wpdb->base_prefix . "modal_survey_participants_details WHERE sid = '" . $vid[ 1 ] . "' AND uid = '" . $vid[ 0 ] . "' ORDER BY time DESC";
	$thissession = $this->wpdb->get_var( $check_session );
	if ( empty( $thissession ) ) {
		...
		$sql_ss = "SELECT samesession, time FROM " . $this->wpdb->base_prefix . "modal_survey_participants_details mspd LEFT JOIN " . $this->wpdb->base_prefix . "modal_survey_participants msp on mspd.uid = msp.autoid WHERE mspd.sid = '" . $vid[ 1 ] . "' AND msp.id = '" . $ms_user->id . "' GROUP BY samesession ORDER BY time DESC";
		$s_sql_ss = $this->wpdb->get_results( $sql_ss );
		print('
...
$surveys = $this->wpdb->get_results( "SELECT msp.autoid, msp.id as uid, DATE_FORMAT( mspd.time,'%Y-%m-%d %H:%i' ) as created, msp.name, msp.email, mspd.ip, COUNT( mspd.aid ) as SUMCOUNT, mss.name as survey, mss.id as sid, mspd.sid as pdsid FROM " . $this->wpdb->base_prefix . "modal_survey_participants msp LEFT JOIN " . $this->wpdb->base_prefix . "modal_survey_participants_details mspd on mspd.uid = msp.autoid LEFT JOIN " . $this->wpdb->base_prefix . "modal_survey_surveys mss on mspd.sid = mss.id GROUP BY mss.id, msp.id" . $limit );
	...
	foreach( $surveys as $sv ) {
		$sql_ss = "SELECT samesession FROM " . $this->wpdb->base_prefix . "modal_survey_participants_details mspd LEFT JOIN " . $this->wpdb->base_prefix . "modal_survey_participants msp on mspd.uid = msp.autoid WHERE mspd.sid = '" . $sv->sid . "' AND msp.id = '" . $sv->uid . "' GROUP BY samesession";
		$s_sql_ss = $this->wpdb->get_results( $sql_ss );
	...
	print('</tbody></table>');
	print('<input type="button" id="delete_allp" class="button button-secondary button-small" value="' . __( 'DELETE SELECTED', MODAL_SURVEY_TEXT_DOMAIN ) . '"><input type="button" id="bulk_export" class="button button-secondary button-small" value="' . __( 'BULK EXPORT SELECTED', MODAL_SURVEY_TEXT_DOMAIN ) . '">');
