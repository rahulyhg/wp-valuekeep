...
<div id="modal_survey_accordion">
			...
			<div class="new_condition_line">
				...
				<input type="text" name="actionvalue" class="actionvalue" value="" />
				<a href="" class="add_condition"><img class="modal_survey_tooltip" title="' . __( 'Add New Condition', MODAL_SURVEY_TEXT_DOMAIN ) . '" src="'.plugins_url( "/assets/img/add.png" , __FILE__ ).'"></a>
			</div>
			<hr>
			...
			if ( $options[ 21 ] != "" ) {
				...
				foreach( $options[ 21 ] as $condkey => $conds ) {
					...
					$newline_str .= str_replace( "|", "'", htmlspecialchars( $conds_c[ 4 ] ) );
					print( '<div class="one_condition_line"><a href="#" class="remove_condition"><img class="modal_survey_tooltip" title="Remove Condition" src="' . plugins_url( '/assets/img/delete.png' , __FILE__ ) . '"></a><input type="hidden" class="packed_cond" value="' . $conds_n . '">' . $newline_str . '</div>' );
				}
			}
...
$ms_settings =   array(
	...
	'drag_drop_upload' => true //Enable Drag & Drop Upload Support
			...
			<div>
				...
				<div>
					<div>
						<label class="text modal_survey_tooltip" title="' . __( 'Enable MailChimp', MODAL_SURVEY_TEXT_DOMAIN ) . '" style="width: 200px;"><input type="checkbox" name="mailchimp_status" class="inputtext addmoreapi_mailchimp admincheckbox" '.$mailchimp.' value="'.$mailchimp_value.'" /> ' . __( 'Enable MailChimp', MODAL_SURVEY_TEXT_DOMAIN ) . '</label>
					</div>
					<div class="aright">
...
<div class="shortcode_section"><a href="#" class="help-dialog button button-primary" data-helpid="help3">' . __( 'Results', MODAL_SURVEY_TEXT_DOMAIN ) . ' </a><a href="#" class="help-dialog button button-primary" data-helpid="help1">' . __( 'Embed Mode', MODAL_SURVEY_TEXT_DOMAIN ) . ' </a><a href="#" class="help-dialog button button-primary" data-helpid="help2">' . __( 'Modal Mode', MODAL_SURVEY_TEXT_DOMAIN ) . ' </a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div>
		...
		$question_options = '<div class="qo-accordion qaa' . ( $key + 1 ) . ' qaa' . $qv->id . '_' . ( $key + 1 ) . '"><h4>' . __( 'Question Options', MODAL_SURVEY_TEXT_DOMAIN ) . '</h4><div><div class="left_qopts">' . $left_que_options . $quploader . '</div><div class="right_qopts">' . $que_options . '</div></div></div>';
		$left_ans_options = "";
		print('<div class="group">
		<h3>' . ( $key + 1 ) . '. ' . __( 'question', MODAL_SURVEY_TEXT_DOMAIN ) . '</h3>
  ...
  <ul class="no-js">
		<li><span class="button button-primary">' . __( 'Export Answers', MODAL_SURVEY_TEXT_DOMAIN ) . '</span>
			<ul>
				...
				$answer_options = '<div class="ao-accordion oaa' . ( $key2 + 1 ) . ' oaa' . $sv->id . '_' . ( $key2 + 1 ) . '"><h4>' . __( 'Answer Options', MODAL_SURVEY_TEXT_DOMAIN ) . '</h4><div><div class="left_aopts">' . $left_ans_options . $uploader . '</div><div class="right_aopts">' . $ans_options . '</div>' . $open_answer_details .'</div></div>';
				...
				if ( $key2 == 0 ) {
					print('<div class="default_answer"><a href="#" class="answer_options" data-aid="' . ($key2+1) . '"><img class="modal_survey_tooltip" title="' . __( 'Answer Options', MODAL_SURVEY_TEXT_DOMAIN ) . '" src="'.plugins_url( "/assets/img/options.png" , __FILE__ ).'"></a>' . $answer_status . '<span class="ans_nmo">'.($key2+1).'.</span> ' . __( 'answer:', MODAL_SURVEY_TEXT_DOMAIN ) . ' <input type="text" name="answer[]" class="answer' . $answer_class . '" data-unique="'.$av->uniqueid.'" id="answer1" ' . $answer_type . ' style="width: 40%;" value="' . htmlspecialchars( $av->answer ) . '" placeholder="' . __( 'no', MODAL_SURVEY_TEXT_DOMAIN ) . '" /><span id="answer_count1" class="answer_count">'.$av->count.' - '.$percentage.'%</span><a class="add_answer"><img class="modal_survey_tooltip" title="' . __( 'Add New Answer', MODAL_SURVEY_TEXT_DOMAIN ) . '" src="'.plugins_url( "/assets/img/add.png" , __FILE__ ).'"></a>' . $answer_options . '</div>');
				}
				else {
					print('</span><div class="added_answers" id="answer_element_'.$sv->id.'_'.($key2+1).'"><a href="#" class="answer_options" data-aid="' . ($key2+1) . '"><img class="modal_survey_tooltip" title="' . __( 'Answer Options', MODAL_SURVEY_TEXT_DOMAIN ) . '" src="'.plugins_url( "/assets/img/options.png" , __FILE__ ).'"></a>' . $answer_status . '<span class="ans_nmo">'.($key2+1).'.</span> ' . __( 'answer:', MODAL_SURVEY_TEXT_DOMAIN ) . ' <input type="text" name="answer[]" data-unique="'.$av->uniqueid.'" class="answer' . $answer_class . '" id="answer'.($key2+1).'" ' . $answer_type . ' style="width: 40%;" value="' . htmlspecialchars( $av->answer ) . '" placeholder="' . __( 'no', MODAL_SURVEY_TEXT_DOMAIN ) . '" /><span id="answer_count'.($key2+1).'" class="answer_count">'.$av->count.' - '.$percentage.'%</span><a class="remove_answer" id="remove_answers_'.$sv->id.'_'.($key2+1).'"><img class="modal_survey_tooltip" title="' . __( 'Remove Answer', MODAL_SURVEY_TEXT_DOMAIN ) . '" src="'.plugins_url( "/assets/img/delete.png" , __FILE__ ).'"></a>' . $answer_options . '</div>');
				}
			}
			print('
		</div>
